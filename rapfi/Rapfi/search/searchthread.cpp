                               ...
                               std::function<void()> onStop)
{
    ...
    for (Pos p : options.restrictMoves) {
        if (main()->board->isInBoard(p))
            main()->board->expandCandArea(p, 4, 4);
    }
    ...
    auto addMoveToRootMoves = [this](Pos m) {
        ...
        if (!main()->searchOptions.restrictMoves.empty()) {
            bool found = false;
            for (Pos rm : main()->searchOptions.restrictMoves) {
                if (rm == m) {
                    found = true;
                    break;
                }
